angular.module('AudioTrackr').controller('TrackController', function($scope, $element) {
	...
	track.stop = function() {
		if ($scope.useAudioTag) {
			...
			track.pauseTime = $scope.aCtx.currentTime - track.startTime;
		}
	};
	...
	function doneLoading() {
		...
		$scope.trackLoad($scope.key, track);
	};
	...
	function loadAndDecode(doneCallback, statusCallback) {
		if ($scope.useAudioTag) {
			...
			audio.addEventListener('canplaythrough', function(e) {
				track.node = $scope.aCtx.createMediaElementSource(audio);
			...
			request.onload = function() {
				...
				$scope.aCtx.decodeAudioData(request.response, function(buffer) {
					track.buffer = buffer;
					doneCallback();
				});
			};
	...
	function addGainNode() {
		track.gainNode = $scope.aCtx.createGain();
		track.node.connect(track.gainNode);
		track.gainNode.connect($scope.master.gainNode);
	}
	
	
	function createAnalyser() {
		var analyser = $scope.aCtx.createAnalyser();
	...
	function playBuffer() {
		track.node = $scope.aCtx.createBufferSource();
		track.node.buffer = track.buffer;
		
		var bufferOffset = track.pauseTime || 0;
		track.startTime = $scope.aCtx.currentTime;
		if (track.pauseTime) {
